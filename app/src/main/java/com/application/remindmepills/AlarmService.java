...
public class AlarmService extends IntentService {
	...
	protected void onHandleIntent(Intent intent) {
		...
		if (matcher.matchAction(action)) {
			if (POPULATE.equals(action)) {
				RemindMePills.dbHelper.populate(Long.parseLong(alarmId), RemindMePills.db);
			...
			if (CANCEL.equals(action)) {
				...
				RemindMePills.dbHelper.shred(RemindMePills.db);
			}			
		}
	}
	...
	private void execute(String action, String... args) {		
		...
		if (alarmMsgId != null) {
			c = RemindMePills.db.query(AlarmMsg.TABLE_NAME, null, AlarmMsg.COL_ID+" = ?", new String[]{alarmMsgId}, null, null, null);
			
		} else {
			c = AlarmMsg.list(RemindMePills.db, alarmId, startTime, endTime, status);
		}
